...
type Surface(size: Size) =
    let safeSize =
        if size.IsValid then size
        else Size.Create(max 1 size.Width, max 1 size.Height)
            ...
            if w = 0 then
                ...
                if cx > x && cx > 0 && cx <= safeSize.Width && y >= 0 && y < safeSize.Height then
                    ...
                    if not cell.IsEmpty then
                        cells.[idx] <- { cell with Grapheme = cell.Grapheme + string c }
    ...
    member _.WriteClipped(rect: Rect, text: string, style: Style) =
        ...
        for c in text do
            if c = '\n' then
                ...
                let w = Grapheme.charWidth c
                if w = 0 then
                    if cx > rect.Left && cx > 0 && cx <= rect.Right && cy >= rect.Top && cy <= rect.Bottom && cx <= safeSize.Width && cy < safeSize.Height then
                        let idx = cy * safeSize.Width + cx - 1
                        let cell = cells.[idx]
                        if not cell.IsEmpty then
                            cells.[idx] <- { cell with Grapheme = cell.Grapheme + string c }
                elif cx >= 0 && cx <= rect.Right && cy >= rect.Top && cy <= rect.Bottom && cx < safeSize.Width && cy < safeSize.Height then
    ...
    member _.DrawBox(rect: Rect, style: Style) =
        if rect.Width > 0 && rect.Height > 0 then
            // Corners
            if rect.Left >= 0 && rect.Left < safeSize.Width && rect.Top >= 0 && rect.Top < safeSize.Height then
                cells.[rect.Top * safeSize.Width + rect.Left] <- Cell.FromChar('┌', style)
            if rect.Right >= 0 && rect.Right < safeSize.Width && rect.Top >= 0 && rect.Top < safeSize.Height then
                cells.[rect.Top * safeSize.Width + rect.Right] <- Cell.FromChar('┐', style)
            if rect.Left >= 0 && rect.Left < safeSize.Width && rect.Bottom >= 0 && rect.Bottom < safeSize.Height then
                cells.[rect.Bottom * safeSize.Width + rect.Left] <- Cell.FromChar('└', style)
            if rect.Right >= 0 && rect.Right < safeSize.Width && rect.Bottom >= 0 && rect.Bottom < safeSize.Height then
            ...
            for x in rect.Left + 1 .. rect.Right - 1 do
                ...
                if x >= 0 && x < safeSize.Width && rect.Bottom >= 0 && rect.Bottom < safeSize.Height then
                    ...
                    if x >= 0 && x < safeSize.Width && y >= 0 && y < safeSize.Height then
                        let cell =
                            if x = rect.Left || x = rect.Right || y = rect.Top || y = rect.Bottom then
                                if x = rect.Left && y = rect.Top then Cell.FromChar('┌', borderStyle)
                                elif x = rect.Right && y = rect.Top then Cell.FromChar('┐', borderStyle)
                                elif x = rect.Left && y = rect.Bottom then Cell.FromChar('└', borderStyle)
                                elif x = rect.Right && y = rect.Bottom then Cell.FromChar('┘', borderStyle)
                                elif y = rect.Top || y = rect.Bottom then Cell.FromChar('─', borderStyle)
                                else Cell.FromChar('│', borderStyle)
                            else
