...
4. **Kitty keyboard flags 4/8/16 not requested.** We push `1+2` only, so we don't get
   _alternate keys_ (4 — shifted + base-layout codepoints, for layout-independent binds),
   _all-keys-as-escape_ (8), or _associated text_ (16). Consequences: `CSI u` keys carry
...
6. **Underline color** (`SGR 58`) isn't emitted (only the underline _styles_ are), and
