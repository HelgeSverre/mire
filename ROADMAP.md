...
| `AppTheme`                        | ✅     | `AppTheme` record (swappable style set: `fg`/`fgMuted`/`border`/`accent`/`selection`/`markdown`/…) + `AppTheme.default` mirroring `Style.*`; KitchenSink and Agent demos build branded instances from `brand/palette.fs`.                                                                            |
