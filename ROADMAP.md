...
- [ ] **Focus manager** — 🟡 _core shipped_: `Mire.Layout.Focus`, a pure `RegionId`-keyed focus ring + modal trap stack (`ofOrder`/`next`/`prev`/`focus`/`pushTrap`/`popTrap`/`isFocused`), routed inside `update`. _Pending:_ demo-proof migration; the runtime-owned + mouse-hit-testing variant is deferred to v0.5
