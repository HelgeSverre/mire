...
| `AppTheme`                        | ✅     | `AppTheme` record (swappable style set: `fg`/`fgMuted`/`border`/`accent`/`selection`/`markdown`/…) + `AppTheme.default` mirroring `Style.*`. The Feed and Spreadsheet demos build branded instances from `brand/palette.fs`; the Agent demo keeps its own richer brand `Theme` + `MarkdownStyle`.          |
