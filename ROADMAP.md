...
- [x] **Framework consolidated** into a single `Mire` project (folders = layers); solution is `Mire` + `Mire.Demo.List` + `Mire.Demo.Agent` + `Mire.Demo.Feed` + `Mire.Demo.Spreadsheet` + `Mire.Demo.Minesweeper` + `Mire.Tests`
- [x] **Test project** — `Mire.Tests` (Expecto) covering `Layout.measure`/`render`, `Diff.compute` (incl. sync-output bracketing + display-width cursor advance), `InputParser`, `Grapheme` width, `TextBuffer`/`Input`, `Focus`, `ScrollView`, mouse/paste/focus decoding, and the `Mire.Demo.Minesweeper` `Board` + `Mire.Demo.Feed` `Feed` helpers (132 tests, all green; `dotnet build Mire.slnx` is warning-clean)
