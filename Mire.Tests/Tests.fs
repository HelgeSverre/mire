    ...
    testList
        ...
        [ test "ASCII is one cell" { Expect.equal (Grapheme.charWidth 'A') 1 "ASCII char is width 1" }
          ...
          test "stringWidth ASCII stays on the fast path (never cached)" {
              let s = "a unique plain ascii label for this test"
              Grapheme.stringWidth s |> ignore
              Expect.isFalse (Grapheme.widthCacheContains s) "ASCII text isn't added to the cache"
          }
