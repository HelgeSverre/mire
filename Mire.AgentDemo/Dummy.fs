...
module Dummy =
    ...
    let private diffLines : DiffLine list =
        [ { Sign = ' '; Text = "| 0x44uy -> Some { Key = ArrowLeft; … }" }
          { Sign = '+'; Text = "| 0x5Auy -> Some { Key = Tab; Modifiers = { None with Shift = true } }" }
          { Sign = ' '; Text = "| 0x48uy -> Some { Key = Home; … }" } ]
    ...
    let respond (text: string) : Response =
        ...
        | "long" | "lorem" -> AppendBlocks [ AssistantMd longProse ]
        ...
        | "tool" ->
            AppendBlocks [ ToolCall("shell", "dotnet build Mire.slnx", Succeeded, "1.2s", "Build succeeded.\n  0 Warning(s)\n  0 Error(s)") ]
        | "tool:error" ->
            AppendBlocks [ ToolCall("shell", "npm test", Failed, "exit 1", "FAIL  src/app.test.ts\n  ✗ renders (12 ms)\n2 failed, 5 passed") ]
        | "tool:run" ->
            RunningTool
                { Name = "shell"; Cmd = "npm install"; RunningOut = "resolving packages…"
                  DelayMs = 1600; FinalStatus = Succeeded; FinalMeta = "4.1s"; FinalOut = "added 312 packages in 4s" }
        | "tools" ->
            AppendBlocks
        ...
        | "bash" ->
            AppendBlocks [ ToolCall("bash", "git status -s", Succeeded, "0.1s", " M Program.fs\n ?? Theme.fs") ]
        | "search" ->
            AppendBlocks [ ToolCall("search", "rg \"Scroll\" -n", Succeeded, "4 matches", "Layout/Layout.fs:33:  | Scroll of Rect * ScrollState\nLayout/Layout.fs:200: | Scroll(viewport,..)\nWidgets/Widgets.fs:108: module Scroll") ]
        | "thinking" -> AppendBlocks [ Thinking thinkingSample ]
        | "diff" -> AppendBlocks [ DiffBlock("Mire.Protocol/InputParser.fs", diffLines) ]
        | "files" | "tree" ->
            AppendBlocks [ FileTree [ "└ Mire.AgentDemo/"; "  Theme.fs"; "  Markdown.fs"; "  Blocks.fs"; "  Dummy.fs"; "  Skills.fs"; "  PromptInput.fs"; "  Program.fs" ] ]
        | "tasks" | "timeline" ->
            AppendBlocks [ TaskTimeline [ "init model", Succeeded; "run dummy", Succeeded; "stream tokens", Running; "resolve tool", Running ] ]
        | "plan" ->
            AppendBlocks [ PlanBlock [ true, "HTML prototype"; true, "Shift+Tab decode"; false, "AgentDemo project"; false, "DEMO-TODOS.md"; false, "docs" ] ]
        | "turn" ->
            AppendBlocks
        ...
        | "usage" | "cost" ->
            AppendBlocks [ TableBlock([ "item"; "tokens"; "usd" ], [ [ "input"; "8,210"; "$0.02" ]; [ "output"; "1,940"; "$0.03" ]; [ "total"; "10,150"; "$0.05" ] ]) ]
        | "image" ->
            AppendBlocks [ Notice(Theme.Warning, "▢ image: chart.png\n[image preview unavailable] — Kitty graphics not wired yet (ROADMAP v0.5).") ]
        | "mention" ->
            AppendBlocks [ AssistantMd "I read @Mire/Layout/Layout.fs and @README.md.\n\nType `@` in the prompt to pick a file — a completion popup appears; arrows + Enter insert it." ]
        | "paste" ->
            AppendBlocks [ Notice(Theme.Info, "Paste a long or multi-line block into the prompt — it collapses into a `[Pasted #N · L chars]` chip, and the content shows as a note when you submit.") ]
        | "permission" ->
            OpenModal { Title = "Permission required"; Intro = "Mire wants to run a tool:"; Command = "rm -rf bin obj && dotnet test"; Risk = Some "medium"; AcceptLabel = "Accept"; DenyLabel = "Deny"; Kind = PermissionModal }
        | "approval" | "approve" ->
            OpenModal { Title = "Approve command?"; Intro = "The agent proposes to run:"; Command = "git push --force origin main"; Risk = Some "high"; AcceptLabel = "Approve"; DenyLabel = "Reject"; Kind = ApprovalModal }
        | "confirm" ->
            OpenModal { Title = "Confirm"; Intro = "Clear the transcript?"; Command = ""; Risk = None; AcceptLabel = "Yes"; DenyLabel = "No"; Kind = ConfirmClearModal }
        | "warning" -> SpawnToast(Theme.Warning, "! warning", "3 files have uncommitted changes")
        | "toast" | "toast:success" -> SpawnToast(Theme.Success, "✓ success", "Tests passed in 1.2s")
        | "toast:info" -> SpawnToast(Theme.Info, "info", "Indexed 1,204 files")
        | "toast:error" -> SpawnToast(Theme.Danger, "✗ error", "Connection refused")
        | "error" -> AppendBlocks [ ErrorBlock "Unhandled exception: Sequence contains no elements\n  at Mire.Layout.measure (Layout.fs:142)" ]
        | "notice" -> AppendBlocks [ Notice(Theme.Info, "This harness is not wired to an LLM. Responses are canned — type `help`.") ]
        | "help" | "?" | "commands" -> AppendBlocks [ helpBlock () ]
        | "clear" | "reset" -> ClearTranscript
        | "panel" | "split" | "sidebar" -> MetaCmd ToggleSidebar
        | "mode" -> MetaCmd CycleMode
        | "skills" | "explore" -> MetaCmd OpenSkills
        | "palette" -> MetaCmd OpenPalette
        | "mcp" -> MetaCmd OpenMcp
        | "welcome" -> MetaCmd ShowWelcome
        | other -> AppendBlocks [ AssistantMd (sprintf "You said: **%s**\n\nThis is a canned harness — type `help` to see what it can show." other) ]
