...
module Markdown =
    ...
    let private styledChars (baseStyle: Style) (text: string) : (char * Style) list =
        ...
        let n = text.Length
        let emit (a: int) (b: int) (st: Style) = for j in a .. b - 1 do res.Add(text.[j], st)
    ...
    let private toPieces (chars: (char * Style) list) : (string * Style * bool) list =
        ...
        let flush () =
            if has then res.Add(sb.ToString(), curStyle, curSpace); sb.Clear() |> ignore; has <- false
        for (c, st) in chars do
            let sp = (c = ' ')
            if has && st = curStyle && sp = curSpace then sb.Append(c) |> ignore
            else
                flush ()
                curStyle <- st; curSpace <- sp; sb.Append(c) |> ignore; has <- true
        flush ()
        List.ofSeq res
    ...
    let private packLines (width: int) (pieces: (string * Style * bool) list) : (string * Style) list list =
        ...
        let mutable curW = 0
        let push () = lines.Add(ResizeArray(cur)); cur.Clear(); curW <- 0
        for (t, st, isSpace) in pieces do
            let w = Grapheme.stringWidth t
            if isSpace then
                if cur.Count = 0 then ()                         // drop leading space
                elif curW + w <= width then cur.Add(t, st); curW <- curW + w
                else push ()                                     // wrap swallows the space
            else
                if curW + w <= width || cur.Count = 0 then cur.Add(t, st); curW <- curW + w
                else push (); cur.Add(t, st); curW <- w
        if cur.Count > 0 then push ()
        if lines.Count = 0 then lines.Add(ResizeArray())
    ...
    let private lineNode (segs: (string * Style) list) : LayoutNode<'msg> =
        ...
        for (t, st) in segs do
            if merged.Count > 0 then
                ...
                if ps = st then merged.[merged.Count - 1] <- (pt + t, ps)
                else merged.Add(t, st)
            else merged.Add(t, st)
    ...
    let wrapLines (width: int) (baseStyle: Style) (text: string) : LayoutNode<'msg> list =
        styledChars baseStyle text |> toPieces |> packLines (max 1 width) |> List.map lineNode
    ...
    let private keywords =
        set [ "let"; "fun"; "match"; "with"; "type"; "module"; "namespace"; "open"; "member"
              "if"; "then"; "else"; "elif"; "for"; "in"; "do"; "while"; "rec"; "mutable"
              "function"; "return"; "const"; "var"; "new"; "of"; "and"; "yield"; "use"
              "true"; "false"; "null"; "private"; "public"; "static"; "import"; "from"; "def"; "class" ]
    ...
    let private highlightCodeLine (raw: string) : LayoutNode<'msg> =
        ...
        let n = raw.Length
        let isIdent c = System.Char.IsLetterOrDigit c || c = '_' || c = '\''
        let mutable i = 0
        while i < n do
            let c = raw.[i]
            if c = '"' then
        ...
        Stack.hstackOf (merged |> Seq.map (fun (t, st) -> Stack.sized Length.Content (Text.text t st)) |> List.ofSeq)

    let private isOrdered (line: string) : bool =
        let t = line.TrimStart()
        let mutable i = 0
        while i < t.Length && System.Char.IsDigit t.[i] do i <- i + 1
    ...
    let render (width: int) (src: string) : LayoutNode<'msg> =
        ...
        let addWrapped (style: Style) (text: string) =
            for n in wrapLines width style text do nodes.Add(n)
        let mutable i = 0
        while i < lines.Length do
            ...
            if line.StartsWith("```") then
                ...
                if line.StartsWith("### ") then nodes.Add(Text.text (line.Substring 4) Theme.heading3)
                elif line.StartsWith("## ") then nodes.Add(Text.text (line.Substring 3) Theme.heading2)
                elif line.StartsWith("# ") then nodes.Add(Text.text (line.Substring 2) Theme.heading1)
                elif line.StartsWith("> ") then addWrapped Theme.muted ("│ " + line.Substring 2)
                elif line.StartsWith("- ") || line.StartsWith("* ") then addWrapped Theme.text ("• " + line.Substring 2)
                elif isOrdered line then addWrapped Theme.text line
                elif line.StartsWith("---") then nodes.Add(Text.text (System.String('─', max 1 width)) Theme.borderStyle)
                elif line.Trim() = "" then nodes.Add(Text.text " " Theme.text)
                else addWrapped Theme.text line
                i <- i + 1
