...
// ── model ────────────────────────────────────────────────────────────────────
type Mode = Normal | AutoAccept | Plan
type Pane = ListPane | PreviewPane
type ButtonFocus = AcceptBtn | DenyBtn
type FocusRegion = PromptFocus | TranscriptFocus

type ModalState = { Spec: ModalSpec; Focus: ButtonFocus }
type PaletteState = { Query: string; Sel: int }
type SkillState = { Sel: int; ListScroll: int; PreviewScroll: int; Pane: Pane }

/// Prompt-driven completion popups (coexist with typing): @file mentions and /slash commands.
type CompletionKind = MentionC | SlashC

/// A fake MCP server for the /mcp manager.
type McpServer = { Name: string; Transport: string; Status: string; Tools: string list }
type McpView = McpList | McpActions | McpTools
type McpState = { View: McpView; ServerSel: int; ActionSel: int; ToolScroll: int }
...
type Toast = { Id: int; Tone: Theme.Tone; Title: string; Body: string; Ttl: int }
type Streaming = { Words: string[]; Index: int; BlockIndex: int }
...
type Key2 =
    | KChar of string
    | KBackspace | KEnter | KEsc | KTab | KShiftTab
...
let private mcpInitial : McpServer list =
    [ { Name = "github"; Transport = "http"; Status = "connected"; Tools = [ "search_issues"; "create_pr"; "list_repos"; "get_file"; "add_comment" ] }
      { Name = "filesystem"; Transport = "stdio"; Status = "connected"; Tools = [ "read_file"; "write_file"; "list_dir" ] }
      { Name = "linear"; Transport = "sse"; Status = "needs-auth"; Tools = [ "list_issues"; "create_issue"; "update_issue" ] }
      { Name = "postgres"; Transport = "stdio"; Status = "disconnected"; Tools = [ "query"; "schema" ] }
      { Name = "context7"; Transport = "http"; Status = "connected"; Tools = [ "resolve_library"; "get_docs" ] } ]
...
let private modeLabel = function Normal -> "normal" | AutoAccept -> "auto-accept" | Plan -> "plan"
let private nextMode = function Normal -> AutoAccept | AutoAccept -> Plan | Plan -> Normal
let private modeStyle = function Normal -> Theme.subtle | AutoAccept -> Theme.okStyle | Plan -> Theme.infoStyle
...
let private contentHeight (m: Model) =
    ...
    m.Transcript |> List.sumBy (fun b -> Layout.contentExtent Direction.Vertical (Blocks.renderBlock w m.Spinner b))

let private maxScroll (m: Model) = max 0 (contentHeight m - viewportRows m)
let private followTail (m: Model) = { m with Offset = maxScroll m }

let private filterCommands (q: string) =
    let ql = q.ToLowerInvariant()
    Dummy.commands |> List.filter (fun (t, d) -> t.Contains(ql) || d.ToLowerInvariant().Contains(ql))

let private addToast (tone: Theme.Tone) (title: string) (body: string) (m: Model) =
    { m with
        Toasts = m.Toasts @ [ { Id = m.NextId; Tone = tone; Title = title; Body = body; Ttl = 28 } ]
        NextId = m.NextId + 1 }

// ── @file mentions ──────────────────────────────────────────────────────────────
let private mentionFiles =
    [ "Mire/Core/Input.fs"; "Mire/Layout/Layout.fs"; "Mire/Renderer/Surface.fs"
      "Mire.AgentDemo/Program.fs"; "Mire.AgentDemo/Theme.fs"; "README.md"
      "ROADMAP.md"; "SPEC.md"; "DEMO-TODOS.md" ]
...
let private activeMention (value: string) : string option =
    ...
    let token = if lastSpace < 0 then value else value.Substring(lastSpace + 1)
    if token.StartsWith("@") then Some(token.Substring(1)) else None
...
let private acceptMention (path: string) (value: string) : string =
    let lastSpace = value.LastIndexOf(' ')
    let prefix = if lastSpace < 0 then "" else value.Substring(0, lastSpace + 1)
    prefix + "@" + path + " "
...
let private activeCompletion (value: string) : (CompletionKind * string) option =
    if value.StartsWith("/") && not (value.Contains(" ")) then Some(SlashC, value.Substring(1))
    else
...
let private completionList (kind: CompletionKind) (q: string) : (string * string * string) list =
    ...
    | SlashC ->
        let ql = q.ToLowerInvariant()
        Dummy.commands |> List.filter (fun (t, _) -> t.Contains(ql)) |> List.map (fun (t, d) -> (t, "/" + t, d))
...
let private refreshCompletion (m: Model) : Model =
    ...
    | Some(kind, q) ->
        ...
        if n = 0 then { m with Completion = None }
        else
            let prev = match m.Completion with Some(k, s) when k = kind -> s | _ -> 0
            { m with Completion = Some(kind, clamp 0 (n - 1) prev) }
...
let private applyResponse (rawText: string) (m: Model) : Model * Cmd<Msg> =
    ...
    | AppendBlocks bs -> followTail { m with Transcript = m.Transcript @ bs }, Cmd.none
    | StreamMarkdown md ->
        let words = md.Split(' ')
        let idx = List.length m.Transcript
        let m' = { m with Transcript = m.Transcript @ [ AssistantMd "" ]; Streaming = Some { Words = words; Index = 0; BlockIndex = idx } }
        followTail m', Cmd.none
    | RunningTool tr ->
        let idx = List.length m.Transcript
        let m' = { m with Transcript = m.Transcript @ [ ToolCall(tr.Name, tr.Cmd, Running, "", tr.RunningOut) ] }
        let cmd =
    ...
    | OpenModal spec -> { m with Overlay = ModalOverlay { Spec = spec; Focus = AcceptBtn } }, Cmd.none
    | ClearTranscript -> { m with Transcript = []; Offset = 0 }, Cmd.none
    | MetaCmd meta ->
        match meta with
        | ToggleSidebar -> { m with SidebarOpen = not m.SidebarOpen }, Cmd.none
        | CycleMode -> { m with Mode = nextMode m.Mode }, Cmd.none
        | OpenSkills -> { m with Overlay = SkillOverlay { Sel = 0; ListScroll = 0; PreviewScroll = 0; Pane = ListPane } }, Cmd.none
        | OpenPalette -> { m with Overlay = PaletteOverlay { Query = ""; Sel = 0 } }, Cmd.none
        | OpenMcp -> { m with Overlay = McpOverlay { View = McpList; ServerSel = 0; ActionSel = 0; ToolScroll = 0 } }, Cmd.none
        | ShowWelcome -> followTail { m with Transcript = m.Transcript @ [ Dummy.welcomeBlock ] }, Cmd.none

let private startCommand (text: string) (m: Model) : Model * Cmd<Msg> =
    let m1 = { m with Transcript = m.Transcript @ [ UserMsg text ]; Tokens = m.Tokens + 120 + text.Length * 3 }
    applyResponse text m1

let private submit (m: Model) : Model * Cmd<Msg> =
    let text = m.Prompt.Value.Trim()
    if text = "" then m, Cmd.none
    else
...
let private acceptCompletion (m: Model) : Model * Cmd<Msg> =
    ...
    | Some(kind, sel), Some(kind2, q) when kind = kind2 ->
        ...
        | Some(value, _, _) ->
            match kind with
            | MentionC -> { m with Prompt = { Value = acceptMention value m.Prompt.Value }; Completion = None }, Cmd.none
            | SlashC -> runCommand value { m with Prompt = PromptInput.empty; Completion = None }
...
let private resolveModal (which: ButtonFocus) (ms: ModalState) (m: Model) : Model * Cmd<Msg> =
    ...
    | AcceptBtn ->
        ...
        | _ ->
            let body = if ms.Spec.Command = "" then "proceeding" else ms.Spec.Command
...
let private scrollBy (d: int) (m: Model) : Model * Cmd<Msg> =
    { m with Offset = clamp 0 (maxScroll m) (m.Offset + d) }, Cmd.none
...
let private updateBase (k: Key2) (m: Model) : Model * Cmd<Msg> =
    ...
    | KChar c ->
        ...
        if c.Contains("\n") || c.Length >= 16 then
            ...
            { m with
                ...
                NextId = id + 1
                Completion = None }, Cmd.none
        else
            refreshCompletion { m with Prompt = PromptInput.append c m.Prompt }, Cmd.none
    | KBackspace -> refreshCompletion { m with Prompt = PromptInput.backspace m.Prompt }, Cmd.none
    | KEnter ->
    ...
    | KTab ->
        match m.Completion with
        | Some _ -> acceptCompletion m
        | None -> { m with Focus = (if m.Focus = PromptFocus then TranscriptFocus else PromptFocus) }, Cmd.none
    | KUp ->
        match m.Completion with
        | Some(kind, s) -> { m with Completion = Some(kind, max 0 (s - 1)) }, Cmd.none
        | None -> scrollBy -1 m
    | KDown ->
        match m.Completion with
        | Some(kind, s) ->
            ...
            let n = List.length (completionList kind q)
            { m with Completion = Some(kind, clamp 0 (max 0 (n - 1)) (s + 1)) }, Cmd.none
        | None -> scrollBy 1 m
    ...
    | KCtrlP -> { m with Overlay = PaletteOverlay { Query = ""; Sel = 0 }; Completion = None }, Cmd.none
    | KCtrlO -> { m with Overlay = SkillOverlay { Sel = 0; ListScroll = 0; PreviewScroll = 0; Pane = ListPane }; Completion = None }, Cmd.none
    | KEsc ->
        match m.Completion with
        | Some _ -> { m with Completion = None }, Cmd.none
        | None -> (match m.Streaming with Some _ -> { m with Streaming = None } | None -> m), Cmd.none
    | _ -> m, Cmd.none

let private updatePalette (k: Key2) (ps: PaletteState) (m: Model) : Model * Cmd<Msg> =
    let setP p = { m with Overlay = PaletteOverlay p }
    match k with
    | KEsc | KCtrlP -> { m with Overlay = NoOverlay }, Cmd.none
    | KChar c -> setP { ps with Query = ps.Query + c; Sel = 0 }, Cmd.none
    | KBackspace ->
        let q = if ps.Query = "" then "" else ps.Query.Substring(0, ps.Query.Length - 1)
        setP { ps with Query = q; Sel = 0 }, Cmd.none
    | KUp -> setP { ps with Sel = max 0 (ps.Sel - 1) }, Cmd.none
    | KDown ->
        let n = List.length (filterCommands ps.Query)
        setP { ps with Sel = clamp 0 (max 0 (n - 1)) (ps.Sel + 1) }, Cmd.none
    | KEnter ->
...
let private updateSkills (k: Key2) (ss: SkillState) (m: Model) : Model * Cmd<Msg> =
    ...
    match k with
    | KEsc | KCtrlO -> { m with Overlay = NoOverlay }, Cmd.none
    | KTab -> setS { ss with Pane = (if ss.Pane = ListPane then PreviewPane else ListPane) }, Cmd.none
    | KUp ->
        if ss.Pane = ListPane then
            let sel = max 0 (ss.Sel - 1)
            setS { ss with Sel = sel; ListScroll = max 0 (sel - 5); PreviewScroll = 0 }, Cmd.none
        else setS { ss with PreviewScroll = max 0 (ss.PreviewScroll - 1) }, Cmd.none
    | KDown ->
        if ss.Pane = ListPane then
            let sel = min (n - 1) (ss.Sel + 1)
            setS { ss with Sel = sel; ListScroll = max 0 (sel - 5); PreviewScroll = 0 }, Cmd.none
        else setS { ss with PreviewScroll = ss.PreviewScroll + 1 }, Cmd.none
    | KPageUp -> setS { ss with PreviewScroll = max 0 (ss.PreviewScroll - 5) }, Cmd.none
    | KPageDown -> setS { ss with PreviewScroll = ss.PreviewScroll + 5 }, Cmd.none
    | KEnter ->
        let name = (List.item ss.Sel Skills.all).Name
        { m with Overlay = NoOverlay; Prompt = PromptInput.append name m.Prompt }, Cmd.none
    | _ -> m, Cmd.none
...
let private updateModal (k: Key2) (ms: ModalState) (m: Model) : Model * Cmd<Msg> =
    ...
    | KLeft | KRight | KTab ->
        { m with Overlay = ModalOverlay { ms with Focus = (if ms.Focus = AcceptBtn then DenyBtn else AcceptBtn) } }, Cmd.none
...
let private performMcpAction (ms: McpState) (m: Model) : Model * Cmd<Msg> =
    ...
    | None -> { m with Overlay = NoOverlay }, Cmd.none
    | Some s ->
        let setStatus st = m.Mcp |> List.map (fun x -> if x.Name = s.Name then { x with Status = st } else x)
        match ms.ActionSel with
        | 0 -> addToast Theme.Success "✓ connected" s.Name { m with Mcp = setStatus "connected"; Overlay = McpOverlay { ms with View = McpList } }, Cmd.none
        | 1 -> addToast Theme.Info "authenticated" ("opened browser for " + s.Name) { m with Mcp = setStatus "connected"; Overlay = McpOverlay { ms with View = McpList } }, Cmd.none
        | 2 -> { m with Overlay = McpOverlay { ms with View = McpTools; ToolScroll = 0 } }, Cmd.none
        | _ ->
            let mcp = m.Mcp |> List.filter (fun x -> x.Name <> s.Name)
            addToast Theme.Warning "uninstalled" s.Name { m with Mcp = mcp; Overlay = McpOverlay { ms with View = McpList; ServerSel = clamp 0 (max 0 (List.length mcp - 1)) ms.ServerSel } }, Cmd.none

let private updateMcp (k: Key2) (ms: McpState) (m: Model) : Model * Cmd<Msg> =
    let setM s = { m with Overlay = McpOverlay s }, Cmd.none
    let n = List.length m.Mcp
    match ms.View with
    | McpList ->
        match k with
        | KEsc -> { m with Overlay = NoOverlay }, Cmd.none
        | KUp -> setM { ms with ServerSel = max 0 (ms.ServerSel - 1) }
        | KDown -> setM { ms with ServerSel = clamp 0 (max 0 (n - 1)) (ms.ServerSel + 1) }
        | KEnter -> if n = 0 then { m with Overlay = NoOverlay }, Cmd.none else setM { ms with View = McpActions; ActionSel = 0 }
        | _ -> m, Cmd.none
    | McpActions ->
        match k with
        | KEsc -> setM { ms with View = McpList }
        | KUp -> setM { ms with ActionSel = max 0 (ms.ActionSel - 1) }
        | KDown -> setM { ms with ActionSel = min (List.length mcpActions - 1) (ms.ActionSel + 1) }
        | KEnter -> performMcpAction ms m
        | _ -> m, Cmd.none
    | McpTools ->
        match k with
        | KEsc -> setM { ms with View = McpActions }
        | KUp -> setM { ms with ToolScroll = max 0 (ms.ToolScroll - 1) }
        | KDown -> setM { ms with ToolScroll = ms.ToolScroll + 1 }
        | _ -> m, Cmd.none
...
let update (msg: Msg) (m: Model) : Model * Cmd<Msg> =
    ...
    | StreamChunk ->
        ...
        | None -> m, Cmd.none
        | Some st ->
            if st.Index >= st.Words.Length then { m with Streaming = None }, Cmd.none
            else
                ...
                let streaming = if nextIdx >= st.Words.Length then None else Some { st with Index = nextIdx }
                followTail { m with Transcript = t'; Streaming = streaming }, Cmd.none
    ...
    | SpinnerTick ->
        let toasts = m.Toasts |> List.choose (fun t -> if t.Ttl <= 1 then None else Some { t with Ttl = t.Ttl - 1 })
        { m with Spinner = m.Spinner + 1; Toasts = toasts }, Cmd.none
    | Resized sz -> { m with Size = sz; Offset = clamp 0 (maxScroll { m with Size = sz }) m.Offset }, Cmd.none
    | Ignore -> m, Cmd.none
...
let private promptBox (m: Model) : LayoutNode<Msg> =
    Box.box Theme.borderStyle
        [ PromptInput.render Theme.accentStyle Theme.text Theme.subtle (placeholder m) m.Prompt ]
...
let private sidebar (m: Model) : LayoutNode<Msg> =
    let taskRows =
        m.Tasks |> List.map (fun (n, s) -> Text.text (sprintf " %s %s" (Blocks.statusGlyph m.Spinner s) n) (Blocks.statusStyle s))
    let fileRows = m.Files |> List.map (fun f -> Text.text (" " + f) Theme.subtle)
    Box.box Theme.borderStyle
        [ Stack.vstack ([ Text.text " tasks" Theme.subtle ] @ taskRows @ [ Text.text "" Theme.text; Text.text " files" Theme.subtle ] @ fileRows) ]

let private transcriptRegion (m: Model) : LayoutNode<Msg> =
    let w = transcriptWrapWidth m
    let border = if m.Focus = TranscriptFocus then Theme.borderFocus else Theme.borderStyle
    let rows = m.Transcript |> List.map (fun b -> Stack.sized Length.Content (Blocks.renderBlock w m.Spinner b))
...
let private skillPanel (ss: SkillState) (m: Model) : LayoutNode<Msg> =
    ...
    let listBorder = if ss.Pane = ListPane then Theme.borderFocus else Theme.borderStyle
    let prevBorder = if ss.Pane = PreviewPane then Theme.borderFocus else Theme.borderStyle
    let skillRows =
        Skills.all
        |> List.mapi (fun i s ->
            let style = if i = ss.Sel then Theme.selection else Theme.muted
            Text.text (sprintf " %s" s.Name) style)
    let listNode = Box.box listBorder [ Scroll.vertical ss.ListScroll (Stack.vstack skillRows) ]
    let selected = List.item ss.Sel Skills.all
    let previewNode = Box.box prevBorder [ Scroll.vertical ss.PreviewScroll (Markdown.render previewW selected.Markdown) ]
    let content =
        Stack.vstackOf
            [ Stack.sized (Length.Cells 1) (Text.text (sprintf " Skill explorer · %d skills" (List.length Skills.all)) Theme.subtle)
              Stack.sized Length.Fill
                (Stack.hstackOf
                    [ Stack.sized (Length.Cells listW) listNode
                      Stack.sized Length.Fill previewNode ])
              Stack.sized (Length.Cells 1) (Text.text " Tab switch pane · ↑↓ navigate · Enter insert · Esc close" Theme.subtle) ]
...
let private modalPanel (ms: ModalState) (m: Model) : LayoutNode<Msg> =
    let spec = ms.Spec
    let acceptStyle = if ms.Focus = AcceptBtn then Theme.selection else Theme.muted
    ...
    let cmdLines = if spec.Command = "" then [] else [ Text.text (sprintf " ❯ %s" spec.Command) Theme.text ]
    let riskLines = match spec.Risk with Some r -> [ Text.text (sprintf " risk: %s" r) Theme.warnStyle ] | None -> []
    let rows =
...
let private renderToast (t: Toast) : LayoutNode<Msg> =
    opaque Theme.borderStyle
        (Stack.vstack [ Text.text (" " + t.Title) (Theme.toneStyle t.Tone); Text.text (" " + t.Body) Theme.muted ])

let private toastLayer (m: Model) : LayoutNode<Msg> =
    let cards =
        m.Toasts
        |> List.collect (fun t -> [ Stack.sized (Length.Cells 4) (renderToast t); Stack.sized (Length.Cells 1) Spacer.spacer ])
    let col = Dock.dock [ Dock.top 1 Spacer.spacer; Dock.fill (Stack.vstackOf cards) ]
    Dock.dock
        [ Dock.right 2 Spacer.spacer
          Dock.right 28 col
...
let private completionPopup (kind: CompletionKind) (sel: int) (m: Model) : LayoutNode<Msg> =
    let q = defaultArg (activeCompletion m.Prompt.Value |> Option.map snd) ""
    let items = completionList kind q
    let title = match kind with MentionC -> " files (@mention) — ↑↓ Enter · Esc" | SlashC -> " slash commands — ↑↓ Enter · Esc"
    let popupW = 46
    ...
    let rows =
        ...
        |> List.mapi (fun i (_, label, sub) ->
            ...
            let node =
                if sub = "" then Text.text (" " + label) style
                else
    ...
    let content = Stack.vstackOf (Stack.sized (Length.Cells 1) (Text.text title Theme.subtle) :: rows)
    let band =
        Dock.dock
...
let private mcpPanel (ms: McpState) (m: Model) : LayoutNode<Msg> =
    let pad w (s: string) = if Grapheme.stringWidth s >= w then s else s + System.String(' ', w - Grapheme.stringWidth s)
    let statusStyle st =
        match st with
    ...
    | McpActions ->
        ...
        let name = match server with Some x -> x.Name | None -> "?"
        let status = match server with Some x -> x.Status | None -> ""
        let actionRows =
            mcpActions
            |> List.mapi (fun i a ->
    ...
    | McpTools ->
        let server = List.tryItem ms.ServerSel m.Mcp
        let name = match server with Some x -> x.Name | None -> "?"
        let tools = match server with Some x -> x.Tools | None -> []
        let toolNodes = tools |> List.map (fun t -> Text.text (sprintf " • %s" t) Theme.text)
        let content =
            Stack.vstackOf
...
let subscriptions (m: Model) : Sub<Msg> list =
    [ yield Sub.TerminalResize Resized
      ...
      let anyRunning =
          (m.Transcript |> List.exists (function ToolCall(_, _, Running, _, _) -> true | _ -> false))
...
let init () : Model * Cmd<Msg> =
    let size = TerminalMode.getTerminalSize () |> Option.defaultValue (Size.Create(96, 32))
    { Size = size
...
let runDump () =
    ...
    dumpNode "B. Markdown kitchen-sink" (Size.Create(60, 24)) (Box.box Theme.borderStyle [ Markdown.render 56 Dummy.markdownKitchenSink ])
    dumpNode "C. Tool calls (ok / error)" (Size.Create(56, 12))
        (Stack.vstack
    ...
    dumpNode "D. Diff card" (Size.Create(64, 7))
        (Blocks.renderBlock 60 0 (DiffBlock("InputParser.fs", [ { Sign = ' '; Text = "context" }; { Sign = '+'; Text = "added line" }; { Sign = '-'; Text = "removed line" } ])))
    dumpNode "E. Table card" (Size.Create(50, 8))
        (Blocks.renderBlock 46 0 (TableBlock([ "ID"; "Title"; "Status" ], [ [ "#1"; "Focus manager"; "open" ]; [ "#2"; "Scroll blit"; "done" ] ])))
    let permSpec = match Dummy.respond "permission" with OpenModal s -> s | _ -> failwith "unreachable"
    dumpNode "F. Permission modal over shell" (Size.Create(72, 20)) (view { sample (Size.Create(72, 20)) with Overlay = ModalOverlay { Spec = permSpec; Focus = DenyBtn } })
    dumpNode "G. Skill explorer overlay" (Size.Create(82, 24)) (view { sample (Size.Create(82, 24)) with Overlay = SkillOverlay { Sel = 0; ListScroll = 0; PreviewScroll = 0; Pane = ListPane } })
    dumpNode "H. Command palette" (Size.Create(72, 22)) (view { sample (Size.Create(72, 22)) with Overlay = PaletteOverlay { Query = "to"; Sel = 0 } })
    dumpNode "I. @mention completion" (Size.Create(72, 20)) (view { sample (Size.Create(72, 20)) with Prompt = { Value = "see @Mire" }; Completion = Some(MentionC, 0) })
    dumpNode "J. /slash completion" (Size.Create(72, 20)) (view { sample (Size.Create(72, 20)) with Prompt = { Value = "/to" }; Completion = Some(SlashC, 0) })
    dumpNode "K. /mcp servers" (Size.Create(72, 18)) (view { sample (Size.Create(72, 18)) with Overlay = McpOverlay { View = McpList; ServerSel = 2; ActionSel = 0; ToolScroll = 0 } })
    dumpNode "L. /mcp actions" (Size.Create(72, 18)) (view { sample (Size.Create(72, 18)) with Overlay = McpOverlay { View = McpActions; ServerSel = 0; ActionSel = 2; ToolScroll = 0 } })
    dumpNode "M. /mcp tools" (Size.Create(72, 18)) (view { sample (Size.Create(72, 18)) with Overlay = McpOverlay { View = McpTools; ServerSel = 0; ActionSel = 2; ToolScroll = 0 } })
    printfn ""
